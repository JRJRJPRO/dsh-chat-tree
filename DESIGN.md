...
**分离**：`detachTarget()` 只在**支线起点**（某条会话自有轮次的第一轮）和**登记过的
对话首条**上给按钮 —— 中间某一轮不是接缝，点了没有意义。拆开后两棵树互不显示。

> ⚠️ 拆出来的会话要把**继承来的轮次改算自有**（`ownTurns` 看 `detached`），否则
> 1-2-4 拆出来只剩一个孤零零的 4，开头凭空少一截。
