...
做法：开着卡片时，在这条必经之路上盖一条透明走廊（`bridgeBox`），压在所有点之上。

- 左缘伸到 `-6`，越过卡片右缘（`-4`）压住缝
- 右缘停在悬停点的**圆边**上：再往右就盖住这个点自己、它就点不动了；
  再往左就露出左邻居命中区的右端（在 `x-5`），白挡

走廊只挡抢夺，不抢悬停。**停住不动超过 260ms 就让位** —— 认定你其实想选走廊底下
那个点，于是 `nodeUnder` 算出鼠标压着谁、直接把悬停交过去。

> ⚠️ 让位时必须自己算一次压着谁。元素在**静止**的鼠标底下出现是不触发 `mouseenter`
> 的，光把走廊撤掉的话，用户得抖一下鼠标才选得中。
