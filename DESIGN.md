...
`reachFor(kind, dotSize, theme)` 算每一端让开多少：圆形让半径，**菱形要让 0.71×直径**
...
| | |
|---|---|
| 形状 | `circle` / `rounded` / `square` / `diamond`，菱形靠 `rotate(45deg)` |
| 颜色 | `#rrggbb`。**派生色一律用 `fade()` 现算** —— 路径垫色、外发光、蓝线都从主色派生 |

> ⚠️ 派生色不许再硬编码。用户把主色改成绿的之后，还留着 `rgba(88,166,255,…)`
> 就会绿一半蓝一半（用例 12 钉着，硬编码回蓝会炸 3 条）。

形状用来区分**是不是压缩节点**，颜色用来区分**状态** —— 所以"普通"和"当前路径"共用
一种形状。host 的 schema 只声明成 `Schema.string()`，合法值由浏览器半的 `FIELDS.accept`
把关：存进来一个认不得的值应该退回默认，而不是把树搞崩。
