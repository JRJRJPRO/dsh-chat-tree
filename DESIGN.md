...
**剪出来的形状和字形描不了边。** 三角是 `clip-path` 剪的，斜边上的 border 会被一起剪掉，
`box-shadow` 同理（整圈发光全被剪没）。所以 `carved`（有 `clip` 或 `glyph`）的节点一律
`borderWidth: 0` + 整块填色，外发光换成跟着轮廓走的 `filter: drop-shadow(...)`。
...
| | |
|---|---|
| 形状 | `circle` / `rounded` / `square` / `diamond`（`rotate(45deg)`）/ `triangle`（`clip-path`），或 `char:<字>` |
...
节点将来的样子，按钮上不写"圆形""菱形"这种字。最后一格是自定义输入框，填一个字符
（emoji 也行）节点就画成它。
