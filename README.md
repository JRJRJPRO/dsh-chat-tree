...
| 在卡片上双击 | 展开：名字进文本框，改树形那几个按钮也在这一档 |
| ＋ | 从这之后接着问；在树根空节点上则是**在同一棵树里新开一条对话** |
| ☆ | 收藏 —— 这个点在树上变成**黄色五角星**，滚到它那一轮时填实 |
| 收藏图标 | 收藏过的点，展开卡片后多出一排图标：12 个预设形状、填一个字（emoji 也行）、或者传张图。颜色也能改 —— 卡片上给了 6 个预设加一个取色盘；想统一换掉所有收藏点的默认色，去设置里的「收藏」那一行 |
| 改名 | 展开后单击文本框就能改；改过之后只有「保存」和「不保存」两条出路 |
| ⇥ | 把这条支线拆成独立的一棵树（头顶有岔路的节点才有） |
| ⇤ | 把拆出去的支线接回原来那棵树（出现在剪缝上） |
| ⊕ | 树根空节点上：把本目录下别的对话**整棵**合并进来，再点一次 ⊖ 拆回去 |
| 灰掉的按钮 | 现在做不了，鼠标停上去写着为什么 |

改名改到一半时整张卡会**锁住**：鼠标走开不关、划过别的点也不换点 —— 否则手一滑，
刚敲的名字就没了。中文输入法打字不受影响（选词那一下的回车归输入法，不当成"确认改名"）。
...
配色有**亮色和暗色两版**，跟着宿主主题自动切换 —— 在设置里切浅色/深色，树立刻跟着换，
不用刷新。同一个色号在白底和深底上观感差很多，所以亮色版整体压了明度、提了饱和度。

设置里一行一种（普通节点 / 当前路径 / 压缩节点 / 空节点 / 收藏），左边挑颜色右边挑形状。
收藏不是第五种节点 —— 它是盖在任何一种节点上的一层记号，所以那一行改的是**默认值**，
在树上单独挑过图标或颜色的点仍按它自己的来。
**单独改过的那一项就钉死了**，不再跟着明暗走；按「重置」交还回去。

树上的中性色（连线、卡片底、文字）一律引用宿主的 `--dsw-alias-*` 变量，所以它们
永远和宿主界面同步。

形状 12 个：圆 / 圆角方 / 方 / 菱形 / 三角 / 倒三角 / 箭头 / 五边形 / 六边形 / 十字 /
四角星 / 沙漏。选择器上直接画出形状本身，还跟着这一行选的颜色走 —— 所以按钮上看到的
就是节点将来的样子。四个角色的形状各自独立。空节点的虚线边是「还没说话」的记号，
不跟着配置走。

> 挑这 12 个的判据是**11px 上还分得出来**（默认节点就这么大）。六边形用的是平顶那版，
> 尖顶的在这个尺寸上和圆几乎没区别；正八边形干脆没加，就是个圆。
