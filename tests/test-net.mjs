...
console.log('用例 1：直连能通就不碰 /remote')
{
	...
	check(calls[0].url === '/plugins/dsh-chat-tree/outlines?cwd=D%3A%2Fx', `地址不对：${calls[0].url}`)
...
console.log('用例 4：被围栏拒了就改走 /remote，成了就记住')
{
	...
	check(calls[1].url === '/remote/plugins/dsh-chat-tree/outlines?cwd=D%3A%2Fx', `换路地址不对：${calls[1].url}`)
	...
	check(calls[0].url === '/remote/plugins/dsh-chat-tree/shape', `地址不对：${calls[0].url}`)
